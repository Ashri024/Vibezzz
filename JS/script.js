...
const focusChange = (value, className) => {
    if (value == 0) {
        ...
        className.classList.remove("activeFooter");
        className.classList.add("passive");
    }
    else {
        //if the element is active
        className.classList.add("activeFooter");
        className.classList.remove("passive");
    }
}
...
const find_next_shuffle = () => {
    ...
    if (shuffle_track == 0) {
        ...
        let last_shuffle_ele2 = Math.floor(Math.random() * (hotSpicy[randomeSong].length));
        ...
        if (last_shuffle_ele2 == shuffle_queue2.items[shuffle_queue2.backIndex - 1] || last_shuffle_ele2 == last_shuffle_ele) {
            last_shuffle_ele2 = Math.floor(Math.random() * (hotSpicy[randomeSong].length));
        }
...
right
    ...
    hot_songs_array.forEach((ele, j) => {
        let play_trigger_play = play_trigger[j].querySelector(".song_title");
...
//RIGHT CONTAINER
let footeractive = document.querySelectorAll(".play_options button")[5];
...
const find_next = (play_list) => {
    ...
    for (let i = 1; i <= 5; i++) {
        if (parseInt(shuffleValue) == 1) {
            ...
            // console.log("here is the next song index " + nextSongIndex + " " + nextSongIndex2);
            ...
            // console.log("currentsong: " + copy_currentSong + ", i: " + i + " currentsong+i: " + (copy_currentSong + i));
...
//Creating the clone of all the songs
        all_song_title.forEach(all_song_title=>{
...
form_onokay.addEventListener("click",()=>{
    ...
    let st7=setTimeout(()=>{
        ...
        else if(window.innerWidth<900 && window.innerWidth>540){
        animation_div.style.transform="scale(1.3)";
        console.log("idk2");

        }
        else if(window.innerWidth<=540 && window.innerWidth>300){
        animation_div.style.transform="scale(1.5)";
        console.log("greater thean 300 but less than 540");

        }
        else if(window.innerWidth<=300){
            console.log("less than 300");
        animation_div.style.transform="scale(1.2)";
        }
        // welcome.style.background="linear-gradient(+50deg, #16003B,#16003B,purple,#16003B,#16003B);";
